...
class MnistDataset(Dataset):
    ...
    def load_data(self) -> None:
        ...
        if self.train:
            ...
            targets_file = os.path.join(self.data_folder, "train_target.pt")
        else:
            images_file = os.path.join(self.data_folder, "test_images.pt")
            targets_file = os.path.join(self.data_folder, "test_target.pt")

        self.images = torch.load(images_file)
        self.target = torch.load(targets_file)
